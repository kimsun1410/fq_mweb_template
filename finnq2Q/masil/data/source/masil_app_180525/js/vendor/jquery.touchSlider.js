...
(function (factory) {
	...
	var touchSlider = {
		...
		init : function () {
			...
			this._timer = null;
      this._nowIndex = null;
      this.slideType = this.opts.slideType;
      this.moveValue =this.opts.moveValue;
			this._tg
      ...
      if(this.opts.autoplay.enable){
        ...
        if(this.opts.slideType == 'lastStyle' && d !== 1 && this.opts.page==this._len && this.moveValue > 0) {
          gap = 0;
        }
        if(this.opts.slideType == 'lastStyle' && d == 0 && this.opts.page==this._len && this.moveValue > 0){
          this.moveValue = 0;
				}
				if(this.opts.slideType == 'lastStyle' && this.moveValue > 0){
          for(var i=0, len = this._len; i<len; ++i) {
            this._start[i] = this._start[i]-this.moveValue;
          }
          this.moveValue = 0;
				}
				for(var i=0, len = this._len; i<len; ++i) {
				...
				if(d !== 0 || (this.opts.slideType == 'lastStyle' && d !== 1 && this.opts.page==this._len) ) {
					this.counter();
				}
			}
		},
    ...
    // 슬라이드 tab 구현을 위한 수정 작업 진행
    go_page : function (i, actionType) {
      var crt = ($.inArray(0, this._pos) / this._view) + 1;
      var cal = crt - (i + 1);
      this._nowIndex = i;
      var action = (actionType === 0 || actionType ) ? actionType : this.speeds;

      while(cal !== 0) {
        if(cal < 0) {
          this.animate(-1, true, action);
          cal++;
        } else if(cal > 0) {
          this.animate(1, true, action);
          cal--;
		...
		counter : function (bool) {
			...
			var setBoolType = this.opts.slideType == 'lastStyle' && this.opts.page==this._len && this.moveValue > 0;
			if($.inArray(0, this._pos) < 0 && setBoolType) {
        this.opts.page = 0;
        this.init();
      }
