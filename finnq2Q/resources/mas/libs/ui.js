...
if(typeof jQuery !== 'undefined'){
	...
	// 19.10.30. HDK
	// Toast Popup UI : 토스트 문자 팝업 공통 추가
	var removeToast;
	function toastPopup(string) {
	    var toast = document.getElementById("toast__popup");

	    toast.classList.contains("reveal") ?
	        (clearTimeout(removeToast), removeToast = setTimeout(function () {
	            document.getElementById("toast__popup").classList.remove("reveal")
	        }, 1500)) :
	...
	// 컨텐츠에 스크롤이 없는 알림성 팝업일 경우 : 모달 레이어 컨텐츠 스크롤 막기
	// 페이지 자체 터치를 막음
	function popupScrollStopOn(eventTarget){
		var eventTarget = eventTarget;
		$(eventTarget).on( 'scroll touchmove mousewheel', function(e){
			e.preventDefault();
			e.stopPropagation();
			return false;
		});
	...
	// 컨텐츠에 스크롤이 없는 알림성 팝업일 경우 : 모달 레이어 컨텐츠 스크롤 해지 (초기화)
	// 페이지 터치 해제
	function popupScrollStopOff(eventTarget){
		var eventTarget = eventTarget;
		$(eventTarget).off('scroll touchmove mousewheel');
	}

	// 클릭한 data- 팝업 호출 (컨텐츠 안에 스크롤 없는 알림성 모달 팝업)
	// 이벤트 발생 버튼 : data-modal-open = "오픈할 모달 팝업 명"
	// 호출되는 팝업  :  data-modal-content = "오픈할 모달 팝업 명"
	// 전달할 매개변수 : this
	function modalPopupOpen(_target) {
		var _target = _target.attr('data-modal-open');
		$('[data-modal-content="' + _target + '"]').addClass('is-visible').focus();
		layerParentPageScrollOn();
	}

	// 공통 모달 팝업 닫기
	function modalPopupClose(_target) {
		_target.parents('[class^=modal]').removeClass('is-visible');
		// popupScrollStopOff('[class^=modal]');
		layerParentPageScrollOff();
	}

	// 레이어 부모 페이지 스크롤 막기
	function layerParentPageScrollOn(){
		$('body, html').addClass('scroll-stop');
	}

	// 레이어 부모 페이지 스크롤 해제
	function layerParentPageScrollOff(){
		$('body, html').removeClass('scroll-stop');
	}
